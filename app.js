...
function showAnalysis(res) {
    ...
    const topicList = document.getElementById('topic-strength-list');
    if (topicList && res.details) {
        const stats = {};
        
        // Data grouping
        res.details.forEach(item => {
            const sub = item.q.sub || 'General';
            if (!stats[sub]) stats[sub] = { correct: 0, wrong: 0, total: 0 };
            
            if (item.status !== 'skipped') {
                stats[sub].total++;
                if (item.status === 'correct') stats[sub].correct++;
                else if (item.status === 'wrong') stats[sub].wrong++;
            }
        });

        // HTML Rendering with Green/Red Bars
        topicList.innerHTML = Object.keys(stats).map(sub => {
            const s = stats[sub];
            // Percentages for the bars
            const corPerc = s.total > 0 ? (s.correct / s.total) * 100 : 0;
            const wrgPerc = s.total > 0 ? (s.wrong / s.total) * 100 : 0;
            const subLabel = SUB_NAMES[sub] || sub;

            return `
                <div class="subject-analysis-item p-3 rounded-2xl hover:bg-gray-50 transition-all">
                    <div class="flex justify-between items-end mb-2">
                        <div>
                            <span class="text-[10px] font-extrabold text-[#4318FF] uppercase tracking-tighter">${subLabel}</span>
                            <h5 class="text-sm font-bold text-[#2B3674]">${s.total} Questions Attempted</h5>
                        </div>
                        <div class="text-right">
                            <span class="text-[10px] font-bold text-[#05CD99]">CORRECT: ${s.correct}</span>
                            <span class="text-[10px] font-bold text-[#EE5D50] ml-2">WRONG: ${s.wrong}</span>
                        </div>
                    </div>
                    
                    <div class="flex w-full h-3 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                        <div class="bg-[#05CD99] h-full transition-all duration-1000" style="width: ${corPerc}%" title="Correct"></div>
                        <div class="bg-[#EE5D50] h-full transition-all duration-1000" style="width: ${wrgPerc}%" title="Wrong"></div>
                    </div>
                    
                    <div class="flex justify-between mt-1 text-[9px] font-bold text-gray-400">
                        <span>ACCURACY: ${Math.round(corPerc)}%</span>
                        <span>${s.total > 0 ? 'SECTIONAL PERFORMANCE' : 'NOT ATTEMPTED'}</span>
                    </div>
                </div>
            `;
        }).join('');
...
async function downloadPerformancePDF() {
    const { jsPDF } = window.jspdf;
    const chartCard = document.querySelector("#perfChart").parentElement;

    if (!chartCard) return showToast("Chart not found!", "error");

    showLoader("Generating A4 Centered Graph...");
    ...
    try {
        ...
        // --- Professional Header ---
        pdf.setFillColor(67, 24, 255);
        pdf.rect(0, 0, pageWidth, 15, 'F');
        pdf.setTextColor(255, 255, 255);
        pdf.setFontSize(12);
        pdf.text("ExamWarmUp - Full Performance Analytics", 10, 10);

        // --- Centering and Rotation Logic ---
        const drawHeight = 180; // Chart thickness (within page width)
        const drawWidth = 240;  // Chart length (within page height)

        // Center Calculation:
        const xPos = (pageWidth / 2) + (drawHeight / 2); 
        const yPos = (pageHeight - drawWidth) / 2 + 10; // Vertical Center
        ...
        const userName = document.getElementById('display-user-name')?.innerText || "Candidate";
        
        // Vertical text on the side
        pdf.text(`Generated by ExamWarmUp for ${userName} | ${dateStr}`, 8, pageHeight / 2, { angle: 90 });
...
function openAiApp() {
    // Aapki AI Mock Test wali web app ka link yahan dalein
    const aiAppLink = "https://adityaprocoders.github.io/AI-Exam-Portal/"; 
    
    // Naya tab mein kholne ke liye
    window.open(aiAppLink, "_blank");
}
